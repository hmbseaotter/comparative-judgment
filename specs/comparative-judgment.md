...
- [x] [P1] `init` against an existing store refuses by name, changes no file, and the three cuts and the load summary both survive; `--force` re-initializes an unjudged store and still refuses a judged one.
